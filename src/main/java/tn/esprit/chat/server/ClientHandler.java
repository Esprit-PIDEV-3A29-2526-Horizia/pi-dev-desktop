...
public class ClientHandler implements Runnable {
    ...
    public void run() {
        try {
            ...
            // La première chose que le client envoie, c'est son nom d'utilisateur
            username = in.readLine();
            System.out.println("👤 " + username + " a rejoint le chat.");
            server.broadcastMessage("👤 " + username + " a rejoint la discussion.", this);

            // Boucle principale : lire les messages du client et les diffuser
            String inputLine;
            ...
            // Nettoyage : fermer les flux et le socket, retirer le client
            try {
                in.close();
                out.close();
                clientSocket.close();
