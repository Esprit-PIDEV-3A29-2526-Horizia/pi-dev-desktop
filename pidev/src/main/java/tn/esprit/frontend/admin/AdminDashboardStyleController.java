...
import tn.esprit.services.Serviceuser; // ou le nom exact de votre service utilisateur
...
public class AdminDashboardStyleController implements Initializable {
    ...
    private Serviceuser utilisateurService = new Serviceuser(); // adapter si nécessaire
    ...
    public void initialize(URL url, ResourceBundle rb) {
        // Vérification admin via SessionManager
        if (!SessionManager.isAdmin()) {
            showAlert("Accès refusé", "Vous devez être administrateur pour accéder à cette page.");
            return;
        }
        ...
        totalUsersLabel.setText(String.valueOf(utilisateurService.afficher().size()));
    ...
    private VBox createCard(Publication p) {
        ...
        Label categorie = new Label(p.getCategorie().getLabel()); // assurez-vous que cette méthode existe
        ...
        // À vous d'ajouter l'action (ex: ouvrir un formulaire d'édition)
