...
public class AjouterPublicationController implements Initializable {
    ...
    private void enregistrer() {
        ...
        if (currentUser != null) {
            p.setUtilisateurId(currentUser.getId());
            String nomComplet = currentUser.getPrenom() + " " + currentUser.getNom();
            p.setAuteur(nomComplet);
        } else {
    ...
    private void goBack() {
        ...
        // Utiliser NavigationManager pour retourner à la liste des publications
        if (SessionManager.isAdmin()) {
            NavigationManager.loadView("/views/admin/GestionPublications.fxml", "Gestion des publications");
        } else {
            NavigationManager.loadView("/views/user/UserPublications.fxml", "Mes publications");
        }
    }
