...
public class MesReservationsvoyController {
    ...
    public void initialize() {
        ...
        if (navbarController != null) {
            ...
            // Activer le bouton "Mes Réservations" dans la navbar
            navbarController.setActiveReservations();
        }

        System.out.println("🔍 MesReservationsvoyController - Utilisateur: " +
                (currentUser != null ? currentUser.getEmail() : "null"));

        if (cbStatut != null) {
            cbStatut.setItems(FXCollections.observableArrayList(
                    "Tous", "Confirmée", "En attente", "Annulée", "Expirée"
            ));
    ...
    private void chargerDonnees() {
        if (currentUser == null) {
            ...
            listeOriginale = new ArrayList<>();
            return;
        }

        int idUser = currentUser.getId();
        System.out.println("📋 Chargement des réservations pour l'utilisateur ID: " + idUser);

        List<Reservation> list = rs.getReservationsParUtilisateur(idUser);
        listeOriginale = (list == null) ? new ArrayList<>() : list;

        System.out.println("✅ " + listeOriginale.size() + " réservations chargées");
    }
    ...
    private VBox creerCardReservation(Reservation res) {
        ...
        Label lblStatut = new Label(normalizeStatutAffichage(statut));
        ...
        String statutUpper = statut.toUpperCase(Locale.ROOT);

        if (statutUpper.contains("CONFIRM") || statutUpper.equals("CONFIRMEE")) {
            Label ok = new Label("✓ Validée");
            ok.setStyle("-fx-text-fill: #94A3B8; -fx-font-style: italic;");
            actions.getChildren().addAll(btnDetails, ok);
        } else if (statutUpper.equals("ANNULEE") || statutUpper.equals("EXPIREE")) {
            Label annule = new Label("✗ Réservation " + (statutUpper.equals("EXPIREE") ? "expirée" : "annulée"));
            annule.setStyle("-fx-text-fill: #e74c3c; -fx-font-style: italic;");
            actions.getChildren().addAll(btnDetails, annule);
        } else {
    ...
    private String normalizeStatutAffichage(String statut) {
        if (statut == null) return "En attente";
        String s = statut.toUpperCase(Locale.ROOT);
        return switch (s) {
    ...
    private void voirDetails(Reservation res) {
        ...
        String statut = (res.getStatut() != null && !res.getStatut().isBlank()) ? res.getStatut() : "En attente";
        Label chip = new Label(normalizeStatutAffichage(statut));
        ...
        Label l2 = new Label("👨 Adultes :");
        l2.setStyle("-fx-text-fill: #334155; -fx-font-weight: bold;");
        Label v2 = new Label(String.valueOf(res.getNb_adultes()));
        v2.setStyle("-fx-text-fill: #0F172A;");

        Label l3 = new Label("👶 Enfants :");
        l3.setStyle("-fx-text-fill: #334155; -fx-font-weight: bold;");
        Label v3 = new Label(String.valueOf(res.getNb_enfants()));
        v3.setStyle("-fx-text-fill: #0F172A;");

        Label l4 = new Label("💰 Prix total :");
        l4.setStyle("-fx-text-fill: #334155; -fx-font-weight: bold;");
        Label v4 = new Label(String.format("%.0f DT", res.getPrix_total()));
        v4.setStyle("-fx-text-fill: #0F172A; -fx-font-weight: bold; -fx-text-fill: #E8B156;");

        Label l5 = new Label("📌 Statut :");
        l5.setStyle("-fx-text-fill: #334155; -fx-font-weight: bold;");
        Label v5 = new Label(normalizeStatutAffichage(statut));
        v5.setStyle("-fx-text-fill: #0F172A;");

        Label l6 = new Label("💳 Paiement :");
        l6.setStyle("-fx-text-fill: #334155; -fx-font-weight: bold;");
        String paymentStatus = (res.getPayment_status() != null) ? res.getPayment_status() : "NON_PAYEE";
        Label v6 = new Label(paymentStatus.equals("PAYEE") ? "Payé" : "Non payé");
        v6.setStyle("-fx-text-fill: " + (paymentStatus.equals("PAYEE") ? "#10B981" : "#F59E0B") + "; -fx-font-weight: bold;");
    ...
    private void handleAnnulation(Reservation res) {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            boolean success = rs.annulerReservation(res.getId());
            if (success) {
