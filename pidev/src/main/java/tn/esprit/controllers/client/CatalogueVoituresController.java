...
public class CatalogueVoituresController {
    ...
    // ✅ Plus besoin de btnRetourAccueil car on utilise la navbar
    ...
    private void allerPlanning() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/client/ClientPlanning.fxml"));
            Parent root = loader.load();
            Stage stage = (Stage) flowPaneVoitures.getScene().getWindow();
            stage.setScene(new Scene(root, 1400, 900));
            stage.setTitle("Horizia - Planning");
        } catch (IOException e) {
            System.err.println("Erreur navigation vers Planning : " + e.getMessage());
        }
    }
    /**
    ...
    private VBox creerCardVoiture(Vehicule voiture) {
        ...
        String imageUrl = (voiture.getPhoto() == null || voiture.getPhoto().trim().isEmpty())
                ? "https://via.placeholder.com/400x250/3D94CA/ffffff?text=Voiture"
                : voiture.getPhoto();
        ...
        try {
            ...
            imageView.setImage(new Image("https://via.placeholder.com/400x250/3D94CA/ffffff?text=Voiture", true));
        }
        ...
        lblKm.setStyle("-fx-font-size: 12px; -fx-text-fill: #9CA3AF;");
        Label lblImmat = new Label("🔖 " + voiture.getImmatriculation());
        lblImmat.setStyle("-fx-font-size: 12px; -fx-text-fill: #9CA3AF;");
        ...
        lblParJour.setStyle("-fx-font-size: 11px; -fx-text-fill: #9CA3AF;");
        ...
        card.setOnMouseEntered(e -> {
            card.setStyle(
                    "-fx-background-color: white;" +
                            "-fx-background-radius: 16;" +
                            "-fx-border-color: #23779C;" +
                            "-fx-border-width: 2;" +
                            "-fx-border-radius: 16;" +
                            "-fx-effect: dropshadow(three-pass-box, rgba(35,119,156,0.3), 20, 0, 0, 8);" +
                            "-fx-cursor: hand;"
            );
    ...
    /**
     * Retour à l'accueil - VERSION CORRIGÉE (utilise la navbar)
     */
    @FXML
