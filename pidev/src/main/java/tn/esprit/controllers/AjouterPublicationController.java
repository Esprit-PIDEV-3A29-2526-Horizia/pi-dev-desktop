...
import tn.esprit.utils.ApiClient;
...
public class AjouterPublicationController implements Initializable {
    ...
    private void setupValidation() {
        descriptionArea.textProperty().addListener((obs, oldVal, newVal) -> {
            ...
            descCount.setStyle(newVal.length() > 900
                    ? "-fx-text-fill: #e74c3c;"
                    : "-fx-text-fill: #95a5a6;");
        });
        ...
        titreField.textProperty().addListener((obs, oldVal, newVal) -> {
            ...
            boolean valid = newVal.length() >= TITRE_MIN
                    && TITRE_PATTERN.matcher(newVal).matches();
            titreError.setVisible(!valid && !newVal.isEmpty());
            titreField.setStyle(valid || newVal.isEmpty()
                    ? ""
    ...
    private void enregistrer() {
        ...
        p.setUtilisateurId(SessionManager.getCurrentUser() != null
                ? SessionManager.getCurrentUser().getId() : 1);
        p.setAuteur(SessionManager.getCurrentUser() != null
                ? SessionManager.getCurrentUser().getPrenom() + " " + SessionManager.getCurrentUser().getNom() : "Anonyme");

        for (Categorie cat : Categorie.values()) {
            if (cat.getLabel().equals(categorieCombo.getValue())) {
                p.setCategorie(cat);
                break;
            }
        }
        ...
        if (!villeField.getText().trim().isEmpty() || !paysField.getText().trim().isEmpty()) {
            ...
            if (!villeField.getText().trim().isEmpty())
                location += villeField.getText().trim();
        ...
        try {
            ...
            alert.setContentText("Publication créée avec succès !");
            alert.showAndWait();
            goBack();
        } catch (Exception e) {
